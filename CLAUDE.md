...
- **The form primitives — `.custom-input`, `.custom-search-field`, `.custom-select`, `.custom-date-range-picker` — are one family** in globals.css: same fill (`#f1f5f9`), same `var(--r-field)` radius, same focus glow, same `--sm`(2rem)/`--md`(2.75rem) heights, and each has the admin-dark + site-dark palettes. Add a new control by following that block, not by inventing per-page classes
  - `--r-field` is **the same 24px as `.btn-*`** on purpose — a control the user operates reads as a control, and at `--sm` the browser clamps it to a pill exactly like `btn-sm`. Their dropdowns and popovers stay on `--r-card`, so an open menu still reads as a panel hanging off a control. Change the token, not the four blocks
